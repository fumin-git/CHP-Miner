...
public abstract class CHPCombineGepDecoder implements Decoder {
    protected CHPGep CHPGep;

    public CHPCombineGepDecoder(CHPGep CHPGep)
    {
        this.CHPGep = CHPGep;
    }
    ...
    protected Expression getExpression(char code)
    {
        ...
        {
            expression = CHPGep.getCombineFunction(code);
        }
    ...
    protected Expression decode0(char[] genes)
    {
        ...
        int geneLength = CHPGep.getCombineGeneLength();

        Expression[] expressions = new Expression[geneNumber];
        //解码每个基因
        for (int i = 0; i < geneNumber; i++)
        {
            if(CHPGep.getCombineMaxFunc() == 0)
        ...
        // 用连接函数将它们连接起来
        int arity = CHPGep.getCombineLinkFunctionArity();
        Expression root = expressions[0];
        int index = 1;
        while (index < expressions.length)
        {
            Expression expression = CHPGep.getCombineLinkFunction();
    ...
    protected Expression decode2(char[] genes, int start)
    {
        ...
        while (!queue.isEmpty())
        {
            ...
            for (int i = 0; i < arity; i++)
            {
                if(numFunctions == CHPGep.getCombineMaxFunc() && reachMax == false)
                {
                    reachMax = true;
                    index = start + CHPGep.combineGeneHead;
                }
                code = genes[index++];
                if (Character.isLowerCase(code)==false && Character.isUpperCase(code)==false && Character.isDigit(code)==false && code<970) //改  是函数
                    numFunctions ++;
